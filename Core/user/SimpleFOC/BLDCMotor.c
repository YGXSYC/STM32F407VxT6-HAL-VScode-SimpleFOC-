...
//	voltage_limit = new_voltage_limit < voltage_limit ? new_voltage_limit : voltage_limit;
	...
	/* 启动 TIM2 三路 PWM 输出（PA1=CH2, PA2=CH3, PA3=CH4）*/
...
void setPhaseVoltage(float Uq, float Ud, float angle_el)
{
	...
	/* 设置三路 PWM 占空比（TIM2: PA1=CH2, PA2=CH3, PA3=CH4）*/
	__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, (uint32_t)(Ta * PWM_Period));
	__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, (uint32_t)(Tb * PWM_Period));
	__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_4, (uint32_t)(Tc * PWM_Period));
}
/******************************************************************************/
float velocityOpenloop(float target_velocity)
{
	...
	now_us = DWT->CYCCNT;
	Ts = (float)(uint32_t)(now_us - (uint32_t)open_loop_timestamp) / (float)SystemCoreClock;
	open_loop_timestamp = (unsigned long)now_us;
