...
void PID_init(void)
{
	...
	error_vel_prev=0;
	output_vel_prev=0;
	pid_vel_timestamp = DWT->CYCCNT;
	
	pid_ang_P=10;
	pid_ang_D=0.5;
	error_ang_prev=0;
	pid_ang_timestamp = DWT->CYCCNT;
}
/******************************************************************************/
...
float PID_velocity(float error)
{
	...
	now_us = DWT->CYCCNT;
	Ts = (float)(uint32_t)(now_us - (uint32_t)pid_vel_timestamp) / (float)SystemCoreClock;
	pid_vel_timestamp = (unsigned long)now_us;
...
float PID_angle(float error)
{
	...
	now_us = DWT->CYCCNT;
	Ts = (float)(uint32_t)(now_us - (uint32_t)pid_ang_timestamp) / (float)SystemCoreClock;
	pid_ang_timestamp = (unsigned long)now_us;
	if(Ts == 0.0f || Ts > 0.5f) Ts = 1e-3f;
